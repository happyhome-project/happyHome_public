...
export function createCloudValidationDependencies(options) {
  ...
  return {
    ...
    async assertSemanticAbsent(identity, probe) {
      ...
      const physical = await invokeTemporary(identity, { action: 'countEsDocuments', runId: probe.runId })
      if (physical?.count !== 0) throw new Error('deleted semantic fixture post remains in ES')
      return { exactAbsent: true, esDocumentCount: 0 }
    },
