...
assert(widgetEditor.includes("sectionDisplayTemplate.value === 'text_note'") && widgetEditor.includes("'text_title'") && widgetEditor.includes("'text_body'") && widgetEditor.includes("'text_topics'") && widgetEditor.includes("'text_location'"), 'WidgetEditor must recognize all text-note locked widgets')
assert(widgetEditor.includes('v-if="!isFixedTemplate"') && widgetEditor.includes('不能新增、删除、改类型或调整顺序'), 'text-note must expose exactly its two fixed widgets')
...
assert(widgetEditor.includes("title=\"纯文字笔记固定控件\"") && widgetEditor.includes('标题、正文、话题和设置地点是纯文字笔记的完整固定结构'), 'WidgetEditor must explain the text-note fixed structure')
