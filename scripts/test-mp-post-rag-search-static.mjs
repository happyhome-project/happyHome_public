...
assertIncludes(searchPage, 'if (!searchSession.isCurrent(request.requestSeq)) return', 'search page stale response drop')
assertIncludes(searchPage, 'const searchSession = createSemanticSearchSession()', 'search submitted-query session')
assertIncludes(searchPage, 'watch(query, (nextDraft) => {', 'search draft edit watcher')
assertIncludes(searchPage, 'searchSession.editDraft(nextDraft)', 'search draft invalidation')
assertIncludes(searchPage, 'searchSession.submit(normalizedQuery)', 'search submitted query binding')
assertIncludes(searchPage, 'searchSession.nextPage(query.value, searchFeed.value.nextSkip)', 'search pagination query binding')
assertIncludes(searchPage, 'query: request.query', 'search request uses submitted query')
assertIncludes(searchPage, 'skip: request.skip', 'search request uses submitted skip')
...
assertIncludes(searchPage, '<ArchiveWaterfall', 'shared archive waterfall')
assertIncludes(searchPage, ':columns="searchFeed.columns"', 'two-column search feed')
assertIncludes(searchPage, 'appendSemanticSearchPage(', 'deduplicated search page append')
assertIncludes(searchPage, '@cover-load="onSearchCoverLoad"', 'search cover load handling')
assertIncludes(searchPage, '@cover-error="onSearchCoverError"', 'search cover recovery')
assertNotIncludes(searchPage, 'class="result-card"', 'legacy single-column search card')
assertNotIncludes(searchPage, 'result-cover-empty', 'generated search cover placeholder')
...
assertIncludes(searchPage, '@post="openSearchCard"', 'search result navigation')
