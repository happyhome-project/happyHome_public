...
for(let attempt=0;attempt<20;attempt++){const bound={runId:probe.runId,communityId:probe.communityId,sectionId:probe.sectionId,postId:probe.postId,outboxId:probe.outboxId};const [e,s]=await Promise.all([invokeAdmin('post.ragTimerEvidenceAdmin',{runId:probe.runId},options,defaultRunner),invokeAdmin('post.ragTimerProbeStatusAdmin',bound,options,defaultRunner)]);evidence=e.functionResult?.evidence;const status=s.functionResult;probeEvidence=advanceProbeTimerEvidence(probeEvidence,evidence,{startedAt,outboxId:probe.outboxId,jobId:status?.job?._id});if(probeEvidence.probeOutboxSeen&&probeEvidence.probeV2JobSeen&&status?.complete)break;await new Promise(resolve=>setTimeout(resolve,5000))}
const status=(await invokeAdmin('post.ragTimerProbeStatusAdmin',{runId:probe.runId,communityId:probe.communityId,sectionId:probe.sectionId,outboxId:probe.outboxId,postId:probe.postId},options,defaultRunner)).functionResult
...
}finally{if(probe)await invokeAdmin('post.ragTimerProbeCleanupAdmin',{runId:probe.runId,communityId:probe.communityId,sectionId:probe.sectionId,postId:probe.postId},options,defaultRunner)}
