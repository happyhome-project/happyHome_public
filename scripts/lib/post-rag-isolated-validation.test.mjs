...
function scenario(overrides = {}) {
  ...
  const deps = {
    ...
    async assertSemanticAbsent() { calls.push(['assertSemanticAbsent']); return { exactAbsent: true, esDocumentCount: 0 } },
...
test('fails closed when semantic absence lacks direct ES zero-document proof', async () => {
  const { deps, probe } = scenario({
    async assertSemanticAbsent() { return { exactAbsent: true } },
  })
  await assert.rejects(
    runIsolatedValidation({ head: 'f16b88f', runId: probe.runId, communityId: 'community-a' }, deps),
...
test('sanitizes evidence to the fixed release-grade schema without secrets or provider details', () => {
  const evidence = sanitizeValidationEvidence({
    ...
    semanticAbsent: { exactAbsent: true, esDocumentCount: 0 },
...
test('temporary handler searches only the deterministic bound probe', async () => {
  ...
  try {
    ...
    const handler = loaded.module.createExactIdValidationHandler({
      ...
      async countEsDocuments(value) { return { count: value.postId === ids.postId ? 0 : 1 } },
    }, { validationToken: 'validation-token-123456', timerToken: 'timer-token-654321' })
    ...
    assert.deepEqual(await handler({ action: 'countEsDocuments', runId, validationToken: 'validation-token-123456' }),
      { count: 0 })
  } finally { await loaded.cleanup() }
})
