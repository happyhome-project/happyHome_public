...
test('release home tabs evidence pins below the fixed masthead', () => {
  ...
  assert.match(source, /Math\.abs\(searchPinnedTabsTop - Number\(searchPinned\.search\?\.bottom \|\| 0\)\) <= 8/)
  assert.match(source, /searchPinnedTabsTop >= Number\(searchPinned\.search\?\.bottom \|\| 0\) - 2/)
  assert.doesNotMatch(source, /searchPinnedTabsTop > Number\(searchPinned\.search\?\.bottom \|\| 0\) \+ 8/)
...
test('release home tabs fixture populates the visible native archive feed', () => {
  ...
  assert.doesNotMatch(fixture, /type: 'realtime'/)
  assert.doesNotMatch(fixture, /action: 'post\.createAdmin'/)
  assert.doesNotMatch(fixture, /action: 'createCollaboration'/)
  assert.doesNotMatch(fixture, /collaborationTemplateId:/)
