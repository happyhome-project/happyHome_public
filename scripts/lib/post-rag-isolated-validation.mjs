...
export function sanitizeValidationEvidence(value) {
  ...
  return {
    ...
    esPhysicalDocumentCount: Number(value?.semanticAbsent?.esDocumentCount ?? -1),
...
export async function runIsolatedValidation(options, deps) {
  ...
  try {
    ...
    const semanticHit = await requireDependency(deps, 'assertSemanticHit')(identity, probe, indexed)
    ...
    const semanticAbsent = await requireDependency(deps, 'assertSemanticAbsent')(identity, probe, removed)
    ...
    if (!evidence.semanticExactHit || !evidence.sourceFieldsVerified || !evidence.semanticExactAbsent
      || evidence.esPhysicalDocumentCount !== 0
