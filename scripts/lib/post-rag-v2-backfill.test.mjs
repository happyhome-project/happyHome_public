...
test('formal v2 backfill requires combined processing and records complete coverage evidence', async()=>{
  const calls=[]; const evidence=await executePostRagV2Backfill({
    ...
    health:async o=>{calls.push(['health',o]);return{totals:{failedCommunityCount:0,failedStateCount:0,pendingJobCount:0,failedJobCount:0,potentialMissingActiveCount:0,coverageRatio:1,activePostCount:4,indexedStateCount:4}}},
...
test('formal v2 backfill fails closed on incomplete coverage or failed jobs', async()=>{
  ...
  await assert.rejects(()=>executePostRagV2Backfill({rebuild,health:async()=>({totals:{failedCommunityCount:0,failedStateCount:0,pendingJobCount:0,failedJobCount:0,potentialMissingActiveCount:1,coverageRatio:.5}})},{maxAttempts:1}),/incomplete/)
  await assert.rejects(()=>executePostRagV2Backfill({rebuild,health:async()=>({totals:{failedCommunityCount:0,failedStateCount:0,pendingJobCount:0,failedJobCount:1,potentialMissingActiveCount:0,coverageRatio:1}})},{maxAttempts:1}),/failed or pending/)
})
