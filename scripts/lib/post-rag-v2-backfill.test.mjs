...
test('formal v2 backfill requires combined processing and records complete coverage evidence', async()=>{
  const calls=[]; const evidence=await executePostRagV2Backfill({
    ...
    health:async o=>{calls.push(['health',o]);return{totals:{failedCommunityCount:0,failedStateCount:0,pendingJobCount:0,retryJobCount:0,processingJobCount:0,failedJobCount:0,eligibleActivePostCount:4,exactSourceVersionCount:4,missingExactSourceVersionCount:0,coverageRatio:1}}},
...
test('formal v2 backfill fails closed on incomplete coverage or failed jobs', async()=>{
  const rebuild=async()=>({totals:{failedCommunityCount:0,failedPostCount:0},workerRounds:[]})
  await assert.rejects(()=>executePostRagV2Backfill({rebuild,health:async()=>({totals:{failedCommunityCount:0,failedStateCount:0,pendingJobCount:0,retryJobCount:0,processingJobCount:0,failedJobCount:0,eligibleActivePostCount:2,exactSourceVersionCount:1,missingExactSourceVersionCount:1,coverageRatio:.5}})},{maxAttempts:1}),/incomplete/)
  await assert.rejects(()=>executePostRagV2Backfill({rebuild,health:async()=>({totals:{failedCommunityCount:0,failedStateCount:0,pendingJobCount:0,retryJobCount:0,processingJobCount:0,failedJobCount:1,eligibleActivePostCount:1,exactSourceVersionCount:1,missingExactSourceVersionCount:0,coverageRatio:1}})},{maxAttempts:1}),/failed or pending/)
})
