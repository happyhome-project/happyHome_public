export async function executePostRagV2Backfill(deps, { maxAttempts = 20 } = {}) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    const exactContract = totals
      && Number.isFinite(totals.eligibleActivePostCount)
      && Number.isFinite(totals.exactSourceVersionCount)
    ...
    if (totals.retryJobCount || totals.processingJobCount) {
      if (attempt < maxAttempts) { await deps.wait?.(); continue }
      throw new Error('v2 backfill health has failed or pending work')
    }
  ...
  throw new Error(`v2 backfill coverage incomplete: missing=${lastHealth?.totals?.missingExactSourceVersionCount ?? 'unknown'}`)
}
