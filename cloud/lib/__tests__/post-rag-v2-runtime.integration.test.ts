...
test('isolated ES-compatible endpoint indexes, replaces and removes a v2 post', async () => {
  ...
  const deps: any = { sink: runtime.sink, loadPost: async () => postExists ? ({ _id: 'post-1', communityId: 'community-1', sectionId: 'section-1', status: 'active' }) : null, loadSection: async () => ({ _id: 'section-1', communityId: 'community-1', status: 'active' }), buildProjection: () => currentProjection, readJob: async () => currentJob, renew: async () => currentJob, complete: async (_id: string, value: any) => { completed.push(value.outcome) }, fail: async () => { throw new Error('unexpected failure') } }
  try {
