...
Prepare also generates the formal release plan and an immutable artifact manifest for every planned cloud function, admin-web, and mini-program package. The manifest binds the run ID, full Git SHA, environment, version and description, builder identities, target set, and stable content digests. Cloud probe tokens remain only inside each controlled local cloud artifact; the ledger, logs, manifest, and production state may contain only the token hash and non-secret build identity.

Publish and same-run retry never skip a remote component from an old passed ledger stage alone. Each cloud function is invoked with the token from its digest-verified local artifact and is skipped only when the fresh response exactly matches its function name, build ID, and source SHA. Missing or changed local artifacts, timeouts, and response mismatches fail closed for that function and cause only that function to deploy, followed by fresh probe verification. Admin-web uses the Aliyun `current` publication marker containing its version ID and content digest; a missing, mismatched, or unreadable marker is `unattestable` and deploys. Targets without a reliable readback mechanism are never guessed to be current. Mini-program upload reuse requires the exact prepared package digest, version, description, run ID, and normalized upload receipt; the DevTools release UI gate remains mandatory.
