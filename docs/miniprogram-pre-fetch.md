...
> 本文是机制与配置参考，不是实时生产证明。下列环境和地址保留为配置示例；2026-08-31 文档复核未登录微信后台重新核验开关、灰度比例或真机命中情况，原先“已配置并验证”的表述缺少可追溯时间，不能据此判断当前状态。生产变更权限及发布证据以 [AGENTS.md](../AGENTS.md) 和 [release gate](release-gate.md) 为准。
...
- 显式 token 过期或无效时始终返回安全空快照，绝不降级成游客响应；用户无 active 社区或快照超过 256KB 时也返回安全空快照，不返回成员内容。
...
- 未登录用户也会在 Home 后台读取微信预拉取；游客快照只包含公共社区，因此可以直接显示公共帖子。
- 已登录用户的本地缓存和微信快照仍只先显示社区/板块框架，帖子必须等 `post.bootstrap` 重新验权后显示。
