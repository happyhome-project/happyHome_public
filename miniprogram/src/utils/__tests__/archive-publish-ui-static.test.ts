...
describe('archive publishing entry', () => {
  ...
  test('text note keeps writing focused and uses a three-stage Figma preview flow', () => {
    const create = read('pages', 'create', 'index.vue')
    const composeStart = create.indexOf('v-if="textNoteStep === \'compose\'"')
    const previewStart = create.indexOf('<view v-else class="text-note-preview-step">', composeStart)
    const previewEnd = create.indexOf('</template>', previewStart)
    const compose = create.slice(composeStart, previewStart)
    const preview = create.slice(previewStart, previewEnd)

    expect(compose).not.toContain('textNoteTopicWidget')
    expect(compose).not.toContain('textNoteLocationWidget')
    expect(preview).toContain('text-note-publish-tools')
    expect(preview.indexOf('textNoteTopicWidget')).toBeLessThan(preview.indexOf('textNoteLocationWidget'))
    expect(preview.match(/variant="image-note-tool"/g)).toHaveLength(2)
    expect(preview).toContain('<TextNoteDeck')
    expect(preview).toContain(':current-page="textNoteCurrentPage"')
    expect(preview).not.toContain('text-note-theme-heading')
    expect(preview).toContain('data-testid="text-note-preview-action"')
    expect(create).toContain("const textNoteStep = ref<'compose' | 'cover' | 'body'>('compose')")
    expect(create).toContain("textNoteStep.value = 'body'")
    expect(create).toContain('textNoteCurrentPage.value = 2')
    expect(create).toContain("textNoteStep === 'cover' ? '下一步'")
    expect(create).toContain("isEditMode ? '保存' : '发布'")
