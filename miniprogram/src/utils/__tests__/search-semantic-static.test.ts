...
describe('public semantic post search page', () => {
  ...
  test('renders truthful post matches through the shared archive waterfall', () => {
    ...
    expect(page).toContain('class="search-results-waterfall"')
    expect(page).toContain(':columns="searchFeed.columns"')
    expect(page).toContain('@post="openSearchCard"')
    expect(page).toContain('@cover-load="onSearchCoverLoad"')
    expect(page).toContain('@cover-error="onSearchCoverError"')
    expect(page).toMatch(/\.search-results-waterfall\s*\{[^}]*margin-right:\s*-24rpx;[^}]*margin-left:\s*-24rpx;/s)
    expect(page).not.toContain('class="result-card"')
    expect(page).not.toContain('result-cover-empty')
  ...
  test('keeps bounded input, paging, navigation, and stale-response safety explicit', () => {
    ...
    expect(page).toContain('openPost(card.postId)')
    expect(page).toContain('/pages/detail/index?postId=${encodeURIComponent(postId)}')
    expect(page).toContain("from '../../utils/semantic-search-feed'")
    ...
    expect(page).toContain('searchSession.nextPage(query.value, searchFeed.value.nextSkip)')
    expect(page).toContain('appendSemanticSearchPage(')
