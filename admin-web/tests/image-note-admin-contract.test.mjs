...
test('固定图文模板控件和话题类型在控件编辑器中可识别但不可改', () => {
  ...
  assert.match(widgetEditor, /v-if="!isImageNoteTemplate"[^>]*@click="addWidget"/)
  assert.match(widgetEditor, /function addWidget\(\)\s*\{\s*if \(isImageNoteTemplate\.value\) return/)
})
